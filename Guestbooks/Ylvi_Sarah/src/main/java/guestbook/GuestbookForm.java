...
class GuestbookForm {
	...
	private final @NotBlank @Pattern(regexp = "^(black|maroon|green|olive|navy|purple|teal|silver|grey|red" +
		"|lime|yellow|blue|fuchsia|aqua|white)$") String colour;

	/**
	 * Creates a new {@link GuestbookForm} with the given name, text and colour. Spring Framework will use this constructor to
	 * bind the values provided in the web form described in {@code src/main/resources/templates/guestbook.html}, in
	 * particular the {@code name}, {@code text} and {@code colour} fields as they correspond to the parameter names of the constructor.
	 ...
	 * @param colour the value to bind to {@code colour}
	 */
	public GuestbookForm(String name, String text, String colour) {
	...
	/**
	 * Returns the value bound to the {@code colour} attribute of the request. Needs to be public so that Spring will
	 * actually consider it for form data binding until
	 * {@link https://github.com/spring-projects/spring-framework/issues/22600} is resolved.
	 *
	 * @return the value bound to {@code colour}
	 */
	 ...
	 * @throws IllegalArgumentException if you call this on an instance without the name, text and colour actually set.
	 */
	GuestbookEntry toNewEntry() {
		return new GuestbookEntry(getName(), getText(), getColour());
	}
}
